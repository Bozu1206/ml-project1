...
class CValidator:
    ...
    def __cross_validation(self, y, x, k_indices, k, param, X=None, Y=None):
        ...
        if self.double == True: 
            self.model[1] = fitters.RidgeRegressionFitter(y_train, x_train, y_test, x_test, param[1], param[0])
        
        if self.model[0] == "lrgd" and self.name == "gamma": 
            self.model[1] = fitters.GradientFitter(y_train, x_train, y_test, x_test, 1000, param, 0)
            
        elif self.model[0] == "lrgd" and self.name == "thresh": 
            self.model[1] = fitters.GradientFitter(y_train, x_train, y_test, x_test, 1000, 0.01, param)
            
        elif self.model[0] == "lrsgd" and self.name == "gamma": 
            self.model[1] = fitters.StochasticGradientFitter(y_train, x_train, y_test, x_test, 10000, param, 0)
            
        elif self.model[0] == "lrsgd" and self.name == "thresh": 
            self.model[1] = fitters.StochasticGradientFitter(y_train, x_train, y_test, x_test, 10000, 0.001, param)
            
        elif self.model[0] == "lsq" and self.name == "thresh": 
            self.model[1] = fitters.LeastSquareFitter(y_train, x_train, y_test, x_test, param)
        
        elif self.model[0] == "rrf" and self.name == "lambda" and not self.double:
            self.model[1] = fitters.RidgeRegressionFitter(y_train, x_train, y_test, x_test, param, 0)
        
        elif self.model[0] == "rrf" and self.name == "thresh" and not self.double:
            self.model[1] = fitters.RidgeRegressionFitter(y_train, x_train, y_test, x_test, 10e-7, param)
            
        elif self.model[0] == "lg" and self.name == "gamma": 
            self.model[1] = fitters.LogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, param, 0.5)
            
        elif self.model[0] == "lg" and self.name == "thresh": 
            self.model[1] = fitters.LogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, 0.005, param)
        
        elif self.model[0] == "rlg" and self.name == "gamma": 
            self.model[1] = fitters.RegLogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, param, 10e-7, 0.5)
            
        elif self.model[0] == "rlg" and self.name == "thresh": 
            self.model[1] = fitters.RegLogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, 0.005, 0.005, 10e-7, param)
            
        elif self.model[0] == "lg" and self.name == "lambda": 
            self.model[1] = fitters.RegLogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, 0.005, 0.005, param, 0.5)
        ...
        true_test_preds[np.where(test_preds == 0)] = -1 # In case of logistic regression
    ...
    def cross_validates(self, ratio, seed, ds_factor):
        f1_test = []        
        y_train, x_train, y_test, x_test = preprocessing.prepare_data(self.x, self.y, seed, ratio, ds_factor)
                ...
                for param2 in self.parameters[1]:
                    for k in range(self.K):
                        _, f1score = self.__cross_validation(
                            y_train, x_train, k_indices, k, (param1, param2), X=x_test, Y=y_test
                        )
