...
def main(): 
    ...
    parser.add_argument('--all', type=bool, default=False,help='Run all our models. If not set, just run our best model (Ridge Regression)')
    ...
    raw_x_train, raw_x_test, raw_y_train, train_ids, test_ids = helpers.load_csv_data(DATAPATH)
    end_time = time.time()
    y_train, x_train, y_test, x_test = preprocessing.prepare_data(raw_x_train, raw_y_train, SEED, SPLIT_RATIO, DOWNSAMPLING_FACTOR)
    ...
    if args.cv:
        ...
        cvv = cv.CValidator(raw_y_train, raw_x_train, 4, (rrf_tresholds, rrf_lambdas), ["rrf", None], costs.compute_rmse, "thresh", True)
        best_tresh, best_lambda, f1_score = cvv.cross_validates(SPLIT_RATIO, SEED, DOWNSAMPLING_FACTOR)
    ...
    if args.all: 
        ...
        sgm = fitters.StochasticGradientFitter(y_train, x_train, y_test, x_test, 10000, 0.001, 0)
        w_sgm, loss = sgm.fit()
        ...
        lg  = fitters.LogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, 0.005, 0.5)
        w_lg, loss = lg.fit()
        metrics.agg_results_and_print_results(lg, y_test, x_test, w_lg)
        
        print("Regularized Logistic Regression")
        rlg = fitters.RegLogisticRegressionFitter(y_train, x_train, y_test, x_test, 1000, 0.005, 10e-4, 0.5)
        w_rlg, loss = rlg.fit()
